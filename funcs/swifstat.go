...
func SwIfInMetrics() (L []*model.MetricValue) {
	if g.Config().Switch.Enabled && len(g.Config().Switch.IpRange) > 0 {
		return swIfInMetrics()
	}
	return
}

func SwIfInSpeedPercentMetrics() (L []*model.MetricValue) {
	if g.Config().Switch.Enabled && len(g.Config().Switch.IpRange) > 0 {
		return swIfInSpeedPercentMetrics()
	}
	return
}

func SwIfOutMetrics() (L []*model.MetricValue) {
	if g.Config().Switch.Enabled && len(g.Config().Switch.IpRange) > 0 {
		return swIfOutMetrics()
	}
	return
}

func SwIfOutSpeedPercentMetrics() (L []*model.MetricValue) {
	if g.Config().Switch.Enabled && len(g.Config().Switch.IpRange) > 0 {
		return swIfOutSpeedPercentMetrics()
	}
	return
}
					...
					if lastIfStatList := lastifmap.Get(chIfStat.Ip); lastIfStatList != nil {
						for _, lastifStat := range *lastIfStatList {
							if ifStat.IfIndex == lastifStat.IfIndex {
								...
								IfHCInOctets := 8 * (float64(ifStat.IfHCInOctets))
								IfHCOutOctets := 8 * (float64(ifStat.IfHCOutOctets))
								if limitCheck(IfHCInOctets, speedlimit) {
									L = append(L, CounterValueIp(ts, ip, "switch.if.In", IfHCInOctets, ifNameTag, ifIndexTag))
								} else {
							...
							if ifStat.IfIndex == lastifStat.IfIndex {
								...
								if limitCheck(IfHCInOctets, speedlimit) {
									if ifStat.IfSpeed > 0 {
										...
										L = append(L, CounterValueIp(ts, ip, "switch.if.InSpeedPercent", InSpeedPercent, ifNameTag, ifIndexTag))
									}
								} else {
									log.Println(ip, ifNameTag, "switch.if.In ", "out of range, value is ", IfHCInOctets)
									log.Println("IfHCInOctets This Time: ", ifStat.IfHCInOctets)
						...
						for _, lastifStat := range *lastIfStatList {
							if ifStat.IfIndex == lastifStat.IfIndex {
								...
								if limitCheck(IfHCOutOctets, speedlimit) {
									L = append(L, CounterValueIp(ts, ip, "switch.if.Out", IfHCOutOctets, ifNameTag, ifIndexTag))
								} else {
							...
							if ifStat.IfIndex == lastifStat.IfIndex {
								...
								if limitCheck(IfHCOutOctets, speedlimit) {
									if ifStat.IfSpeed > 0 {
										...
										L = append(L, CounterValueIp(ts, ip, "switch.if.OutSpeedPercent", OutSpeedPercent, ifNameTag, ifIndexTag))
									}
								} else {
									log.Println(ip, ifNameTag, "switch.if.Out ", "out of range, value is ", IfHCOutOctets)
									log.Println("IfHCOutOctets This Time: ", ifStat.IfHCOutOctets)
